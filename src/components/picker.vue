	...
	export default {
		...
		methods: {
			...
			transformEndFun(toggle) {
				...
				this.timer = setInterval(() => {
					this.transFormList[this.touchObj.touchIndex] -= speet;
					if (parseInt(this.transFormList[this.touchObj.touchIndex] % swiperItemHeight) == 0) {
			...
			transformYFun(touchIndex) {
				...
				if (this.mode == "selector") {
					//单列选择器
					maxTransform = (this.rang.length - 1) * swiperItemHeight;
				} else if (this.mode == "multiSelector") {
					maxTransform = (this.rang[touchIndex].length - 1) * swiperItemHeight;
				} else if (this.mode == "time" || this.mode == "date" || this.mode == "dateTime") {
					maxTransform = (this.myDateRang[touchIndex].length - 1) * swiperItemHeight;
				}
			...
			clearIntervalFun(computedToggle) {
				...
				if ((this.mode == "date" || this.mode == "dateTime") && this.touchObj.touchIndex == 0 && computedToggle) {
					let year = this.myDateRang[0][Math.abs(parseInt(this.transFormList[0] / swiperItemHeight))];
			...
			confirmButtonClickFun() {
				...
				if (this.mode == "selector") {
					...
					value = Math.abs(parseInt(this.transFormList[0] / swiperItemHeight));
				} else if (this.mode == "multiSelector") {
					value = [];
					for (let i = 0; i < this.rang.length; i++) {
						value.push(Math.abs(parseInt(this.transFormList[i] / swiperItemHeight)));
					}
				} else if (this.mode == "time") {
					value = [];
					for (let i = 0; i < this.myDateRang.length; i++) {
						value.push(this.myDateRang[i][Math.abs(parseInt(this.transFormList[i] / swiperItemHeight))]);
					}
					value = value.join(":");
			...
			initFun() {
				...
				if (this.mode == "selector") {
					...
					let transFormY = -value * swiperItemHeight;
				...
				if (this.mode == "multiSelector") {
					...
					this.value && this.value.forEach((item, index) => {
						...
						transFormList[index] = -itemValue * swiperItemHeight;
					})
			...
			computedDayFun(activeYear) {
				let daylength = 31;
				let swiperItemHeight = this.swiperItemHeight;
				let month = Number(this.myDateRang[1][Math.abs(parseInt(this.transFormList[1] / swiperItemHeight))]);
				switch (month) {
					case 1:
					case 3:
					case 5:
				...
				if ((dayList.length - 1) * swiperItemHeight + this.transFormList[2] <= 0) {
					this.transFormList[2] = -(dayList.length - 1) * swiperItemHeight;
				}
