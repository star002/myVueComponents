	...
	export default {
		...
		methods: {
			setCookieFun() {
				...
				this.$util.cookie(this.setCookieName, this.setCookieValue);
				this.setCookieName = "";
				this.setCookieValue = "";
			},
			getCookieFun() {
				...
				this.getCookieValue = this.$util.cookie(this.getCookieName);
			},
			removeCookieFun() {
				let errorList = [];
				...
				this.$util.removeCookieFun(this.removeCookieName);
			},
			getCookieAllFun() {
				this.getCookieAll = JSON.stringify(this.$util.getAllCookieFun()) || "没获取到缓存";
			},
			clearCookieAllFun() {
				this.$util.clearCookieFun();
			}
		}
	}
</script>
